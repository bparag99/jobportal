...
//	@Query("select j from JobApplication j where j.job.id=:jobId  order by j.id")
...
//
//	@Query("select new com.digitalchameleon.jobportal.dto.JobApplicationsListDTO(j.id, j.job.id, j.job.jobTitle, j.coverLetter, f.id, CONCAT(f.firstName,' ', f.lastName), f.userName)  from JobApplication j, Freelancer f where f.id=:frId")
//	List<JobListDTO> findByFreelancerId(@Param("frId") Long frId);

	@Query("select case when count(j)> 0 then true else false end from JobApplication j where j.job.id =:jobId and j.freelancer.id =:freelancerId ")
	boolean existsByJobIdAndFreelancerId(@Param("jobId") Long jobId, @Param("freelancerId") Long freelancerId);
